...
Para apilar múltiples íconos, utilice la clase **icon-stack** en el padre, la **icon-stack-1x** para el ícono de tamaño regular, y **icon-stack-2x** para el ícono más grande. **icon-inverse** se puede usar como un color alternativo para el ícono. Incluso puede agregar clases de íconos [más grandes](#iconos-mas-grandes) en el padre para obtener un mayor control del tamaño. Use **icon-stack-right** para apilar un ícono más pequeño a la derecha de un ícono más grande.
