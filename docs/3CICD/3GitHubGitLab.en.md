# GitHub Actions and GitLab CI (YAML)

## GitHub Actions — `.github/workflows/ci.yml`

```yaml
name: Core Product Test (minimal)
...
jobs:
  build:
    ...
    steps:
      ...
      - name: NuGet.Config (private feeds)
        shell: bash
        run: |
      ...
      # Zip the published output and upload artifacts (FE/BE + DACPACs)
      - name: Package publish + dacpacs
        run: |
          mkdir -p artifacts
          # Publish zips
...
workflow:
  rules:
    - if: $CI_COMMIT_BRANCH == "master"        # runs on pushes to master
    - if: $CI_PIPELINE_SOURCE == "web"         # allows triggering it manually from the UI
...
# ==== Variables and cache (template style) ====
variables:
  BUILD_CONFIG: "Release"
  BASE_VERSION: "1.0.0"
  # Template-style cache
...
# ==== before_script: NuGet.Config + Restore (as in the template) ====
before_script:
  - dotnet --info
  - |
    echo "==> Generate NuGet.Config (private feeds)"
    ...
    EOF
  - |
    echo "==> Restore with local cache"
    dotnet restore "$SOLUTION_PATH" \
      --configfile NuGet.Config \
      --packages "$NUGET_PACKAGES_DIRECTORY"

# ==== PREPARE: compute VERSION/INFOVER and share it with the rest of the jobs ====
prepare_version:
  stage: prepare
  script:
    - |
      ...
      echo "Generated variables: VERSION=$VERSION, INFOVER=$INFOVER"
  artifacts:
    reports:
      dotenv: version.env

# ==== BUILD: build the solution and DB (DACPACs) ====
build_solution:
  stage: build
...
# ==== PUBLISH: publish Frontend and Backend (same paths as in GitHub) ====
publish_frontend:
  stage: publish
...
# ==== PACKAGE: zip the published output + collect DACPACs as artifacts ====
package_artifacts:
  stage: package
  needs:
  ...
  script:
    ...
    - mkdir -p artifacts
    - |
      echo "==> Package publish output (Frontend/Backend)"
